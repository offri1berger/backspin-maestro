const R2 = import.meta.env.VITE_R2_BASE_URL

export const AVATARS: string[] = [
  `${R2}/michael%20jackson.jpg`,
  `${R2}/freddie%20mercury.jpg`,
  `${R2}/bob%20dylan.jpg`,
  `${R2}/elton%20john.jpg`,
  `${R2}/elvis%20presley.jpg`,
  `${R2}/jimmy%20page.jpg`,
  `${R2}/layne%20staley.jpg`,
  `${R2}/kurt%20cobain.jpg`,
  `${R2}/dave%20grohl.jpg`,
  `${R2}/kanye%20west.jpg`,
  `${R2}/eminem.jpg`,
  `${R2}/chris%20martin.jpg`,
  `${R2}/bob%20marley.jpg`,
  `${R2}/tame%20impala.jpg`,
  `${R2}/jimi%20hendrix.jpg`,
  `${R2}/beyonce.jpg`,
  `${R2}/phil%20collins.jpg`,
  `${R2}/madona.jpg`,
  `${R2}/prince.jpg`,
  `${R2}/slash.jpg`,
  `${R2}/tupac.jpg`,
  `${R2}/taylor%20swift.jpg`,
  `${R2}/tom%20yorke.jpg`,
  `${R2}/twenty%20one%20pilots.jpg`,  
  `${R2}/bach.jpg`,     
  `${R2}/beethoven.jpg`,     
  `${R2}/mozart.jpg`,     
  `${R2}/eddie%20vedder.jpg`,
  `${R2}/chris%20cornell.jpg`,
  `${R2}/chester%20bennington.jpg`,
  `${R2}/juice%20wrld.jpg`,
  `${R2}/kendrick%20lamar.jpg`,
  `${R2}/drake.jpg`,
  `${R2}/bruno%20mars.jpg`,
  `${R2}/john%20lennon.jpg`,
  `${R2}/paul%20mcCartney.jpg`,
  `${R2}/gene%20simmons.jpg`,
  `${R2}/ozzy%20ozburm.jpg`,
  `${R2}/steven%20tyler.jpg`,
  `${R2}/frank%20sinatra.jpg`,
  `${R2}/tina%20turner.jpg`,
  `${R2}/stevie%20nicks.jpg`,   
  `${R2}/angus%20young.jpg`,
  `${R2}/ed%20sheeran.jpg`,
  `${R2}/mark%20knopfler.jpg`,
  `${R2}/snoop%20dogg.jpg`,
  `${R2}/the%20weekend.jpg`,
  `${R2}/whitney%20houston.jpg`,
  `${R2}/bono.jpg`,
  `${R2}/james%20hetfield.jpg`,
  `${R2}/amy%20winehouse.jpg`,
  `${R2}/david%20bowie.jpg`,
  `${R2}/eric%20clapton.jpg`,
  `${R2}/sting.jpg`,  
  `${R2}/lady%20gaga.jpg`,
  `${R2}/mr%20shadows.jpg`,
  `${R2}/adam%20levine.jpg`,   
  `${R2}/dan%20reynolds.jpg`,   
  `${R2}/justin%20bieber.jpg`,
  `${R2}/adele.jpg`,     
  `${R2}/katy%20perry.jpg`, 
  `${R2}/rihanna.jpg`,     
  `${R2}/one%20direction.jpg`,
  `${R2}/the%20beatles.jpg`,
];
