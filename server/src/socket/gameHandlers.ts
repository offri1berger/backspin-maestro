...
export const registerGameHandlers = (io: IoServer, socket: IoSocket) => {
  socket.on('card:place', async (payload, cb) => {
    try {
      if (!placeLimiter.allow(socket.id)) { cb({ success: false, error: 'rate_limited' }); return }
  ...
  socket.on('steal:attempt', async (payload, cb) => {
    try {
      if (!stealLimiter.allow(socket.id)) { cb({ success: false, error: 'rate_limited' }); return }
  ...
  socket.on('song:skip', async (cb) => {
    try {
      if (!skipLimiter.allow(socket.id)) { cb({ success: false, error: 'rate_limited' }); return }
      const roomCode = getSocketRoomCode(socket)
      if (!roomCode) { cb({ success: false, error: 'not_in_room' }); return }
